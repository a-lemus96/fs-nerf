...
from data.datasets import LLFF
...
class DatasetSplitter:
    ...
    def split(self):
        ...
        self.test_ids, available_idxs = self._select_pose_based(available_idxs, n_test_samples)
        n_val_samples = int(self.val_ratio * len(self.poses))
        self.val_ids, available_idxs = self._select_pose_based(available_idxs, n_val_samples)
        if self.n_training_views < 0:
            self.train_ids = available_idxs
        else:
            assert self.n_training_views > 0, "ValueError, the specified number of training images must be greater than zero."
            self.train_ids, _ = self._select_pose_based(available_idxs, self.n_training_views)


    def get_datasets(train_img_mode: bool = False, **kwargs):
        """Instantiates and returns datasets based on a prior split. Img mode works for training only."""
        assert self.train_ids is not None, "Split the source data before building the datasets."
        # Get image files
        test_poses = self.poses(self.test_ids)
        test_img_paths = self.img_paths(self.test_ids)
        test_imgs = _load_files(test_img_paths)

        val_poses = self.poses(self.val_ids)
        val_img_paths = self.img_paths(self.val_ids)
        val_imgs = _load_files(val_img_paths)
        
        train_poses = self.poses(self.train_ids)
        train_img_paths = self.img_paths(self.train_ids)
        train_imgs = _load_files(train_img_paths)

        # Instantiate datasets
        white_bkgd = kwargs.get('white_bkgd', False)
        ndc = kwargs.get('ndc', True)
        test_dataset = LLFF(test_imgs, test_poses, self.hwf, white_bkgd, True, ndc)
        val_dataset = LLFF(val_imgs, val_poses, self.hwf, white_bkgd, True, ndc)
        train_dataset = LLFF(train_imgs, train_poses, self.hwf, white_bkgd, img_mode, ndc)
        
        return train_dataset, val_dataset, test_dataset
    
    
    def get_dataloaders(train_batch_size, train_img_mode=False, **kwargs):
        datasets = get_datasets(train_img_mode, **kwargs)
    ...
    def _select_pose_based(self, available_idxs: np.array, n_samples: int):
    ...
    def _load_llff_dataset(self):
        ...
        assert os.path.isdir(base_folder_path), f"LLFF dataset folder {os.path.abspath(base_folder_path)} not found."
        all_scenes = os.listdir(path=base_folder_path)
        assert self.scene in all_scenes, f"Scene '{self.scene}' not found in local LLFF dataset folder."
        ...
        imgs_folder_path = os.path.normpath(os.path.join(base_folder_path, self.scene, "images_8/"))
        assert os.path.isdir(imgs_folder_path), f"Images folder path {os.path.abspath(imgs_folder_path)} not found."
        self.img_paths = [os.path.abspath(os.path.join(imgs_folder_path, f))
                 for f in sorted(os.listdir(imgs_folder_path))
                 if f.endswith(('JPG', 'jpg', 'png'))]
        assert len(img_paths) == poses.shape[-1], \
    ...
    @staticmethod
    def __normalize(v: np.array) -> np.array:
        """
        Normalizes a vector.
        ------------------------------------------------------------------------
        Args:
    ...
    @staticmethod
    def __viewmatrix(z: np.array, up: np.array, pos: np.array) -> np.array:
        """
        ...
        """
        z = DatasetSplitter.__normalize(z)
        y = up
        x = DatasetSplitter.__normalize(np.cross(y, z))
        y = DatasetSplitter.__normalize(np.cross(z, x))
        matrix = np.stack([x, y, z, pos], axis=1)

        return matrix

    @staticmethod
    def __avg_pose(poses: np.array) -> np.array:
        """
        ...
        viewdir = DatasetSplitter.__normalize(poses[:, :3, 2].sum(0))
        up = poses[:, :3, 1].sum(0)
        c2w = np.concatenate([DatasetSplitter.__viewmatrix(viewdir, up, center), hwf], 1)

        return c2w

    @staticmethod
    def __recenter_poses(poses: np.array) -> np.array:
        """
        ...
        c2w = DatasetSplitter.__avg_pose(poses) # average pose
    ...
    def postprocess_poses(
        ...
        if recenter:
            poses = DatasetSplitter.__recenter_poses(poses)
